...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        full_object_detection predictShape(
            const unsigned char* imageBuffer,
            const int width,
            const int height,
            const dlib::rectangle rect)
        {
            dlib::array2d<unsigned char> img(height, width); // grayscale image
            unsigned long position = 0;
            for (int row = 0; row < height; ++row) {
                for (int col = 0; col < width; ++col) {
                        // we get mirrored grayscale image buffer so image is mirrored again to fix it.
                    img[row][width - col - 1] = imageBuffer[position++];
                }
            }
            return (*this)(img, rect);
        }
